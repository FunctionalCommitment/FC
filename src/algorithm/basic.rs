...
pub(crate) fn hash_to_r(
    ...
    for (i, row) in b_matrix.iter().enumerate() {
        for (j, element) in row.iter().enumerate() {
            let input = element.as_ref();
            if !input.iter().all(|&x| x == 0) {
                // record the position and value of the non-zero element
                tmp.extend_from_slice(&i.to_le_bytes());
                tmp.extend_from_slice(&j.to_le_bytes());
                let mut output = vec![0u8; input.len() * 8];
                LittleEndian::write_u64_into(input, &mut output);
                tmp.extend_from_slice(&output);
            }
        }
    }
    ...
    for (i, row) in b_matrix.iter().enumerate() {
        ...
        for (j, element) in row.iter().enumerate() {
            ...
            if !input.iter().all(|&x| x == 0) {
                // record the position and value of the non-zero element
                b_vec_ser.extend_from_slice(&i.to_le_bytes());
                b_vec_ser.extend_from_slice(&j.to_le_bytes());
                let mut output = vec![0u8; input.len() * 8];
                LittleEndian::write_u64_into(input, &mut output);
                b_vec_ser.extend_from_slice(&output);
            }
        }
...
/// The efficient version of hash_to_r for svc. 
/// The output of the function is the same as that of hash_to_r
pub(crate) fn hash_to_r_svc(
    com: &Fq12,
    I: &[usize],//for svc
    y_vec: &[G1Affine],
) -> Vec<FrRepr> {
    ...
    //add a b_matrix
    for (i, j) in I.iter().enumerate(){
            let input = FrRepr([1, 0, 0, 0]).as_ref();
            // record the position and value of the non-zero element
            tmp.extend_from_slice(&i.to_le_bytes());
            tmp.extend_from_slice(&j.to_le_bytes());
            let mut output = vec![0u8; input.len() * 8];
...
pub fn to_fixed_length_binary_vec(value: &usize, length: &usize) -> Vec<u8> {//low index contains high significant bit
...
pub fn binary_vec_to_decimal(bits: &[u8]) -> usize {
