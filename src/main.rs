...
fn test_svc(exp: u32, batchsize: usize){
    println!("========================SVC=====================");
    let n: usize = 2_usize.pow(exp);
    let s: usize = batchsize;//batch size>1
    ...
    let start = time::now();//get the start time
    let pi_i = svc::open(&pp, &kzg_pp, &i, &a_vec);
    let end = time::now();//get the end time
    ...
    let mut a_i = a_vec[i];
    let start = time::now();//get the start time
    let resulti = svc::verify(&pp, &kzg_pp, &com, &i, a_i, &pi_i);
    let end = time::now();//get the end time
    println!("the verification result of a proof is {:?}, the required time is {:?}", resulti, end-start);

    /*generate a batch proof*/
    let mut vecI:Vec<usize> = vec![];
    let mut subvec_value: Vec<G1> =vec![];
    let mut subvec_value_affine: Vec<G1Affine> =vec![];
    // let mut b_matrix:Vec<Vec<FrRepr>> = vec![];
    for i in 0..batchsize {
        vecI.push(i);

        subvec_value.push(a_vec[i].clone().into());
        subvec_value_affine.push(a_vec[i].clone());
    ...
    let start = time::now();//get the start time
    // let batch_pi = fc::batchOpen(&pp, &kzg_pp, &com, &a_vec, &b_matrix, &subvec_value_affine);
    let batch_pi = svc::batch_open(&pp, &kzg_pp, &com, &a_vec, &vecI, &subvec_value_affine);
    let end = time::now();//get the end time
    println!("Open subvector of size {} in {:?}",batchsize, end-start);

    /*verify a batch proof*/
    let start = time::now();//get the start time 
    let res = svc::batch_verify(&pp, &kzg_pp, &com, &vecI, &subvec_value_affine, &batch_pi);
    let end = time::now();//get the end time
    println!("Batch verify result is {}, the required time is {:?}", res, end-start);
...
fn test_mc(exp: u32){
    println!("=============================two-tiered SVC=============================");
    let N: usize = 2_usize.pow(exp);
    let n: usize = 2_usize.pow(exp>>1);
    //generate public parameter
    let start = time::now();//get the start time
    let (prover_params, verifier_params) = algorithm::pointproofs::pointproof_setup(n);
    let pp = algorithm::svc::gen(&n);
    let kzp_size = 2*n+1;
    let kzg_pp = algorithm::kzg::setup(&kzp_size);
    let end = time::now();//get the end time
    ...
    let j: usize = 0;
    let start = time::now();//get the start time
    let pi_i = svc::open(&pp, &kzg_pp, &i, &c_vec);
    ...
    let start = time::now();//get the start time
    let resulti = svc::verify(&pp, &kzg_pp, &com, &i, c_i, &pi_i);
    ...
    let batch_pi = svc::batch_open(&pp, &kzg_pp, &com, &c_vec, &vecI, &out_sub_value_affine);
    let end = time::now();//get the end time
    ...
    let res2 = svc::batch_verify(&pp, &kzg_pp, &com, &vecI, &out_sub_value_affine, &batch_pi);
